...
/* importando o modelo */
import { Projeto, Usuario} from "./models/model"
...
async function listUser(req, res){ 
    console.log("Requisição de listagem recebida."); //Para debug somente.
    let users = await service.listAl();  
    let json_user_list = JSON.stringify(users);
    res.setHeader('Content-Type', 'application/json');
    res.end(json_user_list);     
}
